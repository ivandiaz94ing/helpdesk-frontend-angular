...
export class AdminReportesComponent implements OnInit {
  ...
  public pieChartData = computed(() => {
    ...
    const hardware = lista.filter(t => t.category === 'Hardware_dañado').length;
    const software = lista.filter(t => t.category === 'Solicitud_de_software').length;
  ...
  public barChartData = computed(() => {
    return {
      ...
      datasets: [
        { data: [this.ticketsPendientes()], label: 'Pendientes', backgroundColor: '#fb923c' },
        { data: [this.ticketsResueltos()], label: 'Resueltos', backgroundColor: '#34d399' }
      ]
    };
  });
